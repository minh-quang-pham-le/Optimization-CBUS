...
def is_valid_route(route, n):
    ...
    - Mỗi điểm trả (i + n) phải xuất hiện sau điểm đón (i).
    """
...
def is_within_capacity(route, n, k):
    """
    Kiểm tra ràng buộc sức chứa của xe:
    - Số hành khách trên xe không được vượt quá k tại bất kỳ thời điểm nào.
    """
    current_load = 0
    for point in route:
        if point <= n:  # Điểm đón
...
max_iterations = 200
tabu_size = 10
best_route, best_cost = tabu_search(n, k, distance_matrix, initial_route, max_iterations, tabu_size)

print("Initial Greedy Route:", initial_route)
print("Best Route after Tabu Search:", best_route)
print("Best Cost after Tabu Search:", best_cost)